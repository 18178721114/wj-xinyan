...
class TzjCallback extends \Air\Libs\Controller
{
    const SWITCHS = 0;
    public function run()
    {
        ...
        if ($str == $signature) {
            ...
            Logger::error("wechat_event_msg:{$openid};postObj:" . json_encode($postObj), 'wechat_gr_callback');
            //判断该数据包是否是订阅的事件推送
            if (strtolower($postObj->MsgType) == 'event') {
                ...
                if (strtolower($postObj->Event) == 'subscribe') {
                    ...
                    } elseif (strpos('pre' . $eventKey, 'TZJ_')) {
                        $info = $this->handleFD16($eventKey);
                    } elseif (!empty($eventKey)) {
                        $info = $this->handleTextMsg();
                    } else {
                        $msg = '欢迎关注体知健视网膜报告。如果您对您的报告有任何问题，回复消息可以咨询我们的客服人员。';
    ...
    private function getPatientInfoLink($prefix = '8989', $sn = '', $org_id = 0)
    {
        ...
        if ($sn) {
            ...
            //使用第三方公众号启动相机
            if (WXUtil::thirdQrMatchReport($user['org_id'], 'tizhijian')) {
                $msg = '亲，您扫错二维码了，请联系工作人员获取正确的二维码。';
                $str = sprintf(WechatMsgTemplate::MSG_COMMON_TEXT, $toUser, $fromUser, time(), $msg);
                return $str;
            }
            // 判断用户扫描与机构报告是否一致
            $qr_match_report = WXUtil::qrMatchReport($user['org']['customer_id'], 'tizhijian');
        ...
        CheckLog::addLogInfo(0, 'wechat_tzj_callback_event', $check_log_remark, 0, '', $code);
        ...
        if ($org && $org['config']['rigister_miniprogram'] || $user['org']['config']['rigister_miniprogram']) {
            ...
            $template_id = TZJ_WX_REGISTER_TEMPLATE_ID;
            if (ENV == 'test') {
                ...
                WechatUserCheck::sendRegisterMiniprogram($wx_util, $template_id, $openid, WXUtil::h5Url2Miniprogram($url), '体知健', REGISTER_WX_APPID);
                $info = '';
            }
        } else {
