...
let footer_text = decodeURI(args[5]) || '';
...
let port = Math.floor(Math.random() * 6) + 9220;
let random = (Math.floor(Math.random() * 10));

let host = '172.17.170.154';
if ((random % 3) == 0) {
    host = '172.17.170.135'; //3分之1
} else if ((random % 7) == 1) {
    host = '172.17.170.154'; //3分之1
} else {
    host = '172.17.170.155'; //3分之1
}
http.get('http://' + host + ':'+port+'/json/version', (response) => {
  let todo = '';
      ...
      (async () => {
        ...
        if (footer_text === '') { // 隐藏简版，隐藏完整版和简版
          footerTemplateOrg =
            ...
            '</div>' +
            '</div>';
        } else if (parseInt(footer_text) !== 3 && product === 'Airdoc鹰瞳') { // 不隐藏
          // SME定制处理
          if(footer_text.split(' ').length===3){
            const [name, uuid, footerConf] = footer_text.split(' ')
            footer_text = `${name} ${uuid}`
            let footer_reference = ''
            let footer_tel = ''
            let footer_page = ''
            let footer_user_info = ''
            if(footerConf.includes('footer_reference')){
              footer_reference = `
                <div style="color: #e7e8ec;margin: 0 4mm -0.25mm 4mm; font-size:1.625mm; border-bottom: 0.1mm solid #e7e8ec;">注：本报告仅供临床医生参考。</div>              \`
              `
            }
            if(footerConf.includes('footer_tel')){
              footer_tel = `
                  <div style="justify-content: center; margin: auto">
                      <span style="color:#e74478;font-weight:500;">${logoImg}</span>&nbsp;&nbsp;${telephone_text}
                  </div>
              `
            }
            if(footerConf.includes('footer_page')){
              footer_page = `
            ...
            if(footerConf.includes('footer_user_info')){
              footer_user_info = `
                <span style="justify-content: flex-start;  margin-right: auto">${footer_text}</span>
              `
            }
            footerTemplateOrg = `
              <div style="width:100%; position:relative;padding:0 1%;box-sizing:border-box;">
                ${footer_reference}
                <div
                  style="font-size:2mm;color:#394868; display: flex; align-items: center; margin-left: 4mm; margin-right: 4mm;">
                  ${footer_user_info}
                  ${footer_tel}
                  ${footer_page}
                </div>
